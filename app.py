...
# Hat Listesi (Kısaltılmış örnek, seninkinin tamamı buraya gelecek)
TUM_HATLAR = ["B5", "93", "97", "14L2", "6F", "1A", "B24", "38", "97G"] # Buraya senin uzun listeyi koyacağız
...
# --- ARAYÜZ TASARIMI ---
st.title("🚌 CNTOOTURK AKILLI TAKİP")
st.markdown("Plaka (16M...) veya Hat Kodu (B5) giriniz.")

# Giriş Kutusu
giris = st.text_input("Sorgu Ekranı:", placeholder="Örn: 16M10171 veya B5")
btn_ara = st.button("SORGULA 🔍", type="primary")
...
if btn_ara and giris:
    ...
    if len(giris) > 4 and giris[0].isdigit():
        ...
        st.info(f"🔍 '{hedef_plaka}' tüm sistemde aranıyor...")
        
        bulunan = None
        # Hızlı Tarama
        for hat in TUM_HATLAR + ["HAT SEÇİLMEMİŞ", "SERVİS DIŞI"]:
            data = veri_cek(hat)
            for bus in data:
        ...
        if bulunan:
            st.success(f"✅ BULUNDU! Hat: {bulunan['bulunan_hat']}")
            
            # Kart Görünümü
            col1, col2 = st.columns(2)
            col1.metric("Hız", f"{bulunan.get('hiz')} km/s")
            col1.metric("Anlık Yolcu", f"{bulunan.get('seferYolcu')}")
            col2.metric("Toplam Ciro", f"{bulunan.get('gunlukYolcu')}")
            col2.write(f"👮 **{bulunan.get('surucu') or 'Bilinmiyor'}**")
            ...
            try:
                ...
                st.caption(f"📍 Konum: {loc.address}")
            except: pass
            
        else:
            st.error("❌ Araç bulunamadı veya kontak kapalı.")

    # --- SENARYO 2: HAT SORGUSU ---
    else:
        st.info(f"📡 '{giris}' hattı verileri çekiliyor...")
        data = veri_cek(giris)
        
        if data:
            toplam_yolcu = sum(b.get('gunlukYolcu', 0) for b in data)
            st.metric("Bu Hattaki Toplam Yolcu", f"{toplam_yolcu} Kişi", delta=f"{len(data)} Araç Aktif")
            ...
            st.warning("Hat bulunamadı veya aktif araç yok.")
